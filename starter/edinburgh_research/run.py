...
- Party size: {_PARTY_SIZE} people
- Date: {_EVENT_DATE} (Saturday)
- Time: {_EVENT_TIME}
- Location: near {_AREA} station, Edinburgh
- Budget: maximum £{_BUDGET} total
...
_PLANNER_SYSTEM = f"""
    ...
    '  \'Call venue_search(near={_AREA}, party_size={_PARTY_SIZE}, budget_max_gbp={_BUDGET}. '
    'Then call get_weather(city="edinburgh", date={_EVENT_DATE}. '
    "Then call calculate_cost using the venue_id value returned by venue_search, party_size={_PARTY_SIZE}, "
    ...
    'Fixed constants: date={_EVENT_DATE}, time={_EVENT_TIME}, party_size={_PARTY_SIZE}. '
...
_EXECUTOR_SYSTEM = f"""
    ...
    "    date                 <- fixed constant '{_EVENT_DATE}'\n"
    "    time                 <- fixed constant '{_EVENT_TIME}'\n"
    "    party_size           <- fixed constant {_PARTY_SIZE}\n"
    ...
    for i, sg in enumerate(subgoals):
        ...
        if i > 0 and sg1_tool_outputs:
            ...
            sg.description = (
                ...
                f"  date='{_EVENT_DATE}', time='{_EVENT_TIME}', party_size= {_PARTY_SIZE}\n"
...
def _build_fake_client() -> FakeLLMClient:
    ...
    from starter.edinburgh_research.tools import calculate_cost, get_weather, venue_search

    # ── derive honest values from real tools ──────────────────────────
    venues = venue_search(_AREA, _PARTY_SIZE, _BUDGET)
    weather = get_weather("edinburgh", "2026-04-25")
    venue = venues.output["results"][0]
    cost = calculate_cost(venue["id"], _PARTY_SIZE, 3, "bar_snacks")

    venue_id = venue["id"]
    venue_name = venue.get("venue_name") or venue.get("name")
    venue_address = venue.get("venue_address") or venue.get("address")
    condition = weather.output["condition"]
    temperature_c = weather.output["temperature_c"]
    total_gbp = cost.output["total_gbp"]
    deposit_gbp = cost.output["deposit_required_gbp"]
    # ──────────────────────────────────────────────────────────────────
    ...
    search_call = ToolCall(
        id="c1",
        name="venue_search",
        arguments={"near": _AREA, "party_size": _PARTY_SIZE, "budget_max_gbp": _BUDGET},
        #                   ^^^^                                               ^^^^^^^
    )
    weather_call = ToolCall(
        id="c2",
        name="get_weather",
        arguments={"city": "edinburgh", "date": _EVENT_DATE},
        #                                       ^^^^^^^^^^^
    )
    cost_call = ToolCall(
        id="c3",
        name="calculate_cost",
        arguments={
            "venue_id": venue_id,  # ← Use the variable you already computed
    ...
    flyer_call = ToolCall(
        ...
        arguments={
            "event_details": {
                ...
                "date": _EVENT_DATE,  # ← Use constant
                "time": _EVENT_TIME,  # ← Use constant
                "party_size": _PARTY_SIZE,
                "condition": condition,  # ← from real tool
                "temperature_c": temperature_c,  # ← from real tool
                "total_gbp": total_gbp,  # ← was 497, now real
                "deposit_required_gbp": deposit_gbp,  # ← from real tool
            }
        },
    )
    complete_call = ToolCall(
        id="c5",
        name="complete_task",
        arguments={"result": {"flyer": "workspace/flyer.html", "venue": venue_id}},
        #                                                                ^^^^^^^^^
    )
...
def _tools_are_implemented() -> tuple[bool, str]:
    ...
    for name, call in [
        ("venue_search", lambda: venue_search("Haymarket", _PARTY_SIZE, 1000)),
        ("get_weather", lambda: get_weather("edinburgh", "2026-04-25")),
        ("calculate_cost", lambda: calculate_cost("haymarket_tap", _PARTY_SIZE, 3)),
    ]:
        try:
            call()
